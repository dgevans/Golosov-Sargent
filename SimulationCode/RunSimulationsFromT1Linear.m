function  [sHist,gHist,xHist,RHist, xNLHist, RNLHist,TauHist,YHist,TransHist,...
...
for i=1:NumSim-1
    ...
    [PolicyRulesInit]=GetInitialApproxPolicy([xNL RNL s_] ,domain,PolicyRulesStore);
    [PolicyRulesNL, ~,exitflag,~]=CheckGradNAG(xNL,RNL,s_,c,V,PolicyRulesInit,Para);
    ...
    % x' - u_c_2* btildprime
    xprime=PolicyRules(11:12);
    RprimeNL=PolicyRulesNL(end-3:end-2);
    % x' - u_c_2* btildprime
    xprimeNL=PolicyRulesNL(end-1:end);
    % btildprime - x'/u_c2
    ...
    xNLHist(i+1)=xprimeNL(sHist(i+1)) ;
