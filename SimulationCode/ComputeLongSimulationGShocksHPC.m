...
Para.saveSimPath= ['~/Golosov-Sargent/Data/temp/BootStrapGShocks.mat'];

parfor ctrb=1:K
    rHist0 = rand(NumSim,1);
    CoeffFileName='~/Golosov-Sargent/Data/temp/csigmaLow.mat';
    SD(ctrb)=RunSimulationsFromT1Alt(CoeffFileName,x0,R0,NumSim,Para,rHist0);
end
save(Para.saveSimPath,'SD');
%figure()
for i=1:K
AutoCorr(i)=corr(SD(i).TauHist(:end),SD(i).TauHist(2:end-1))
