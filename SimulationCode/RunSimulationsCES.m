function  [SimData]=RunSimulationsCES(CoeffFileName,btild0,c10guess,c20guess,NumSim,Para,rHist0)
...
[x,~,exitflagv0,~,~] = fminunc(@(x)  getValue0CES(x, btild_1,s_,Para,c,V),[ c10guess c20guess],options);
if ~(exitflagv0==1)
[x,~,exitflagv0,~,~] = fminunc(@(x)  getValue0CES(x, btild_1,s_,Para,c,V),[ 1 1/Para.RMax],options);
end
...
if ~(exitflagv0==1)
    ...
    lb=[0.001 0.001];
    ub=[10 10];
    [x,fval,exitflagv0,output,lambda]  =fmincon(@(x) getValue0CES(x, btild_1,s_,Para,c,V),[ x ],[],[],[],[],lb,ub,[],opts);
...
DenL1=n1*theta_1+n2*theta_2*(theta_2*R0/theta_1)^(1/gamma);
l10=(TotalResources)/(DenL1);
l20= (theta_2*R0/theta_1)^(1/gamma) * l10;
xprime0=-(c20-c10)*(c20^(-sigma))-(l10^(1+gamma)*R0-l20^(1+gamma))+btild_1*c20^(-sigma);
...
for i=1:NumSim-1
    ...
    [PolicyRules, ~,exitflag,~]=CheckGradNAGCES(x,R,s_,c,V,PolicyRulesInit,Para);
    ...
    % Int-Rates
    IntNum=1/(c2Hist(i)^(sigma)); %marginal utility of consumption (Agent 2) in s_
