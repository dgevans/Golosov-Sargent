...
for s=1:S
        [PolicyRulesInit_tomorrow]=GetInitialApproxPolicy([xprime(s) Rprime(s) s] ,domain,PolicyRulesStore);
        [PolicyRulestom, V_new,exitflag,fvec]=CheckGradNAG(xprime(s),Rprime(s),s,c,V,PolicyRulesInit_tomorrow,Para)     ;
        c2prime(s,:)=PolicyRulestom(S+1:2*S);
       BondPrice(s)=beta*sum(P(s,:).*c2prime(s,:).^(-sigma))./c2(s).^(-sigma); 
end
...
C1=(alpha_1/alpha_2)*C2;
 ...
 ff=@(c2) (alpha_2/alpha_1)^sigma*c2+c2+g-theta_2+((1-psi)./(psi)).*c2.^sigma;
c2=fsolve( ff ,alpha_2*ones(1,S));
L2=1-(1-psi)*c2.^(sigma)/(psi);
L1=0;
end    
...
NormDecomp=[NormDecomp -Tau(end)*100+Tau(1)*100 -BondPrice(end)^(-1)*100+BondPrice(1)^(-1)*100];
