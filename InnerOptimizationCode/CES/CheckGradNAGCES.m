function [PolicyRules, V_new,exitflag,fvec]=CheckGradNAGCES(xx,RR,ss,c,VV,zInit,Para)
...
%using nag algorithm find solutions to the FOC
 ...
 %z=zInit;
[z, fvec,~,ifail]=c05qb('BelObjectiveUncondGradNAGCES',zInit,'xtol',1e-10);
...
[ xprimeMat,gradxprime ] = computeXprimeCES( c1,gradc1,c2,gradc2,Rprime,gradRprime,l1,gradl1,l2,gradl2,...
...
while  (sum(upperFlags~=upperFlagsOld) + sum(lowerFlags~=lowerFlagsOld))>0
    ...
    if sum(intFlags) ~= S
      ...
      %  SolveKKTWith2Shocks;
       [z, fvec,~,ifail]=c05qb('resFOCCES_alt',zInit);
...
%Return policies.
btildprime = xprime./(c2(1,:).^(-sigma));
V_new=-Value3CEScont(z(1:2*S-1));
