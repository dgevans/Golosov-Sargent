    ...
    clear all
    EndoVarList={'c1','c2','l1','l2','b2'}
    ShocksList={'g','theta_1','theta_2'}
    %VarList={'c1','c2'}
    ParamList={'sigma','gamma','beta','alpha_1','alpha_2','phi_1','phi_2'};
    EqName{1}='I'
    ExConstr{1}='ss_lambda_I*(( ss_c2*exp(c2)- ss_c1*exp(c1)) + beta*ss_b2*exp(b2)+ss_c1^sigma*ss_l1^(1+gamma)*exp(sigma*c1+(1+gamma)*l1)-ss_c2^sigma*ss_l2^(1+gamma)*exp(sigma*c2+(1+gamma)*l2)-ss_b2*exp(b2_))'
    EqName{2}='B1'
    ExConstr{2}='(ss_lambda_B1*((ss_c1*exp(c1_))^(-sigma)-(ss_c1*exp(c1))^(-sigma)))/(ss_c1^(-sigma))'
    EqName{3}='B2'
    ExConstr{3}='(ss_lambda_B2*((ss_c2*exp(c2_))^(-sigma)-(ss_c2*exp(c2))^(-sigma)))/(ss_c2^(-sigma))'
    EqName{4}='W'
    ...
    EqName{5}='R'
    ExConstr{5}='ss_lambda_R*(ss_c1*exp(c1)+ss_c2*exp(c2)-ss_theta_1*exp(theta_1)*ss_l1*exp(l1)-ss_theta_2*exp(theta_2)*ss_l2*exp(l2)+ss_g*exp(g) -beta*ss_b2*exp(b2) + ss_b2*exp(b2_))'
    EqName{6}='O'
    ExConstr{6}='(alpha_1*((ss_c1*exp(c1))^(1-sigma)/(1-sigma)- (ss_l1*exp(l1))^(1+gamma)/(1+gamma) )+ alpha_2*((ss_c2*exp(c2))^(1-sigma)/(1-sigma)- (ss_l2*exp(l2))^(1+gamma)/(1+gamma)   ))*ss_c1^(sigma)'
    
    fid = fopen('SageCodeOpenEconomy.txt', 'wt');
