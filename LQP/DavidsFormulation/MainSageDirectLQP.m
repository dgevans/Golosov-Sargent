    ...
    %VarList={'c1','c2'}
    ParamList={'sigma','gamma','beta','alpha_1','alpha_2','phi_1','phi_2','omega_c1','omega_c2','eta_1','eta_2','omega_l1','omega_l2','kappa_1','kappa_2','delta_1','delta_2'};
    EqName{1}='I'
    ...
    EqName{4}='W'
    ExConstr{4}='ss_lambda_W*((1)*(exp(l1))^(gamma)* (exp(c1))^(sigma)/(exp(theta_1)) - (exp(l2))^(gamma)*(exp(c2))^(sigma)/(exp(theta_2))*(1) )'
    EqName{5}='R'
    ...
    EqName{6}='O'
    ExConstr{6}='(omega_c1*(c1-kappa_1*l1-eta_1*theta_1 )^2 + omega_c2*(c2-kappa_2*l2-eta_2*theta_2 )^2 +omega_l1*(l1-delta_1*theta_1)^2 + omega_l2*(l2-delta_2*theta_2)^2  )'
    EqName{7}='QQ'
    ExConstr{7}='alpha_1*(ss_Q*exp(Q)-beta)'


    fid = fopen('SageCodeLQP.txt', 'wt');
