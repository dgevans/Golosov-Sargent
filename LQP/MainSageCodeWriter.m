...
ShocksList={'g'}
%VarList={'c1','c2'}
ParamList={'sigma','gamma','beta','theta_1','theta_2','alpha_1','alpha_2','phi_1','phi_2'};
...
EqName{4}='W'
ExConstr{4}='ss_lambda_W*(phi_1*(exp(l1))^(gamma)* (exp(c1))^(sigma) * theta_2-(phi_2*(exp(l2))^(gamma)*(exp(c2))^(sigma)*theta_1 ))'
EqName{5}='R'
ExConstr{5}='ss_lambda_R*(ss_c1*exp(c1)+ss_c2*exp(c2)-theta_1*ss_l1*exp(l1)-theta_2*ss_l2*exp(l2)-ss_g*exp(g))'
EqName{6}='O'
