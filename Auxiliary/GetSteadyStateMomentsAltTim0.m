...
disp(u2btild_1)
% c1 and c2 solve
f = @(R) -funeval(c(1,:)',V(1),[u2btild_1 R]);
[R0,~,exitflagv0] = fminsearch(f,mean(Para.RGrid));
c10=max(getValueC1(u2btild_1,R0,1,Para ),.0001);
c20=R0^(-1)*c10;
...
u2btildprime0=(((1-Para.psi)/(Para.psi))*BracketTerm+u2btild_1/(Para.beta*Para.psi)+R0-1)*Para.psi;
...
Trans0=c20-l20.*ul20./uc20;
tau0=1-(ul20/(Para.theta_2*uc20));
Para.g(1)+(Para.n1+Para.n2)*Trans0-tau0*(Para.n1*l10*Para.theta_1+Para.n2*l20*Para.theta_2)
...
tau1=1-(ul2/(Para.theta_2*uc2));
 Trans=c2-l2.*ul2./uc2;
Para.g(1)+(Para.n1+Para.n2)*Trans-tau1*(Para.n1*l1*Para.theta_1+Para.n2*l2*Para.theta_2)
