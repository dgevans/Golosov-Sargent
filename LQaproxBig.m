...
f = @(c2bar,l1bar,l2bar) [ c2bar - c1bar + l1bar^(1+gamma)*c1bar^sigma - l2bar^(1+gamma)*c2bar^sigma - bbar*(1-beta);...
    c1bar+c2bar+gbar-z1bar*l1bar-z2bar*l2bar;
    l1bar^gamma*c1bar^sigma/z1bar - l2bar^gamma*c2bar^sigma/z2bar];

x = fsolve(@(x)f(x(1),x(2),x(3)),[0.5;0.5;0.5]);
c2bar = x(1);
l1bar = x(2);
l2bar = x(3);
