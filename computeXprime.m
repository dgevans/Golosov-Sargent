function [ xprime,gradxprime ] = computeXprime( c1,gradc1,c2,gradc2,R,gradR,l1,gradl1,l2,gradl2,...
                                          P,sigma,psi,beta,s_)
...
%   scalars or have dimension 3x2
    ...
    %Now compute xprime
    xprime = psi*c2.^(-sigma)./(beta*Euc2) + (1-psi)*l2./(1-l2)...
             -(1-psi)*R.*l1./(1-l1)+psi*c1.*c2.^(-sigma)-psi*c2.^(1-sigma);
    %Now compute the gradient
    gradxprime = ( -sigma*psi*c2.^(-sigma-1)./(beta*Euc2) + (sigma*psi*c2.^(-2*sigma-1).*P.*beta)./((beta*Euc2).^2)...
                   -sigma*psi*c2.^(-sigma-1).*c1-(1-sigma)*psi*c2.^(-sigma)).*gradc2...
                +(sigma*psi*c2.^(-sigma).*c2alt.^(-sigma-1).*beta.*Palt)...
                 ./((beta*Euc2).^2)*gradc2alt+psi*c2^(-sigma).*gradc1...
                +(1-psi)*gradl2./((1-l2).^2)-(1-psi)*R.*gradl1./((1-l1).^2)...
                -(1-psi)*l1.*gradR./(1-l1);
end
