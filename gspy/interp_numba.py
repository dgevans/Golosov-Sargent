...
class CubicSpline2d(object):
    ...
    def __init__(self, lx, ly, ux, uy, nx, ny, alpha, beta):
        ...
        self.hx = double(0.)
        self.hy = double(0.)
        self.xgrid = double_1d(np.linspace(self.lx, self.ux, self.nx + 1))
        self.ygrid = double_1d(np.linspace(self.ly, self.uy, self.ny + 1))
        self.c_mat = double_2d(np.empty((2, 2)))

    @f8(f8, i8, i8)
    def _calcuv_i(self, x, x_or_y, i):
        """
        ...
        x_or_y: Pass 0 for x and 1 for y
        ...
        if x_or_y == 0:
            ...
            print 'YOU HAVE A VALUE ERROR!! THINGS ARE NOT WORKING!'
            print "ValueError: Variable x_or_y must have value equal to \
                             either 'x' or 'y'"
    ...
    def coefs(self, z):
        ...
        """
        x = double_1d(np.squeeze(self.xgrid))
        y = double_1d(np.squeeze(self.ygrid))
        ...
        if nx * ny == len(z):
            print 'YOU HAVE A VALUE ERROR!! THINGS ARE NOT WORKING!'
            print "ValueError: z should have a size of  nx * ny. Cannot proceed"
        if x[2] - 2 * x[1] + x[0] > 1e-6:
            print 'YOU HAVE A VALUE ERROR!! THINGS ARE NOT WORKING!'
            print "ValueError: x should be evenly spaced.  Cannot proceed"
        if y[2] - 2 * y[1] + y[0] > 1e-6:
            print 'YOU HAVE A VALUE ERROR!! THINGS ARE NOT WORKING!'
            print "ValueError: y should be evenly spaced.  Cannot proceed"
        ...
        #  change
        cstar = double_2d(np.empty((nx + 3, ny + 1)))

        for i in range(y.size):
            ctemp = double_1d(calccubicspline1d(x, z[:, i], self.alpha, self.beta,
                                      self.hx))
        ...
        c_mat = double_2d(np.empty((nx + 3, ny + 3)))
        # c_mat = np.empty((nx + 3, ny + 3))

        for i in range(nx + 3):
            ctemp = double_1d(calccubicspline1d(y, cstar[i, :], self.alpha,
                                      self.beta, self.hy))
            c_mat[i, :] = ctemp
    ...
    #                 * self._calcuv_i(x, 0, row + lx) * self._calcuv_i(y, 1, column + ly)
